...
class RoomsComfortsRepository(BaseRepository):
    ...
    async def set_room_comforts(self, room_id: int, comforts_ids: list[int]) -> None:
        ...
        current_comforts_ids = res.scalars().all()
        ids_to_delete = list(set(current_comforts_ids) - set(comforts_ids))
        ids_to_insert = list(set(comforts_ids) - set(current_comforts_ids))
        if ids_to_delete:
            delete_rooms_comforts = delete(self.model).filter(
                self.model.room_id == room_id, self.model.comfort_id.in_(ids_to_delete)
            )
            await self.session.execute(delete_rooms_comforts)
        if ids_to_insert:
            insert_rooms_comforts = insert(self.model).values(
